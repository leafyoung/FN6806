 ...
 public:
  ...
  friend std::ostream& operator<<(std::ostream& out, const Task& task) {
    out << "sleep: " << task.sleep_in_milliseconds.count() << ", time range: "
        << std::chrono::duration_cast<std::chrono::milliseconds>(task.start_time.time_since_epoch())
               .count()
        << '-'
        << std::chrono::duration_cast<std::chrono::milliseconds>(task.wake_time.time_since_epoch())
               .count();
